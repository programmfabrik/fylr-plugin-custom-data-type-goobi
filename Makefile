ZIP_NAME ?= "customDataTypeGoobi.zip"
PLUGIN_NAME = "custom-data-type-goobi"

# coffescript-files to compile
COFFEE_FILES = CustomDataTypeGoobi.coffee \
	CustomDataTypeGoobiFacet.coffee \
	GoobiUtil.coffee
...
build: clean buildinfojson ## clean, compile, copy files to build folder
			...
			cat src/tmp/CustomDataTypeGoobi.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeGoobi.js
			cat src/tmp/CustomDataTypeGoobiFacet.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeGoobi.js
			cat src/tmp/GoobiUtil.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeGoobi.js

			cp src/updater/GoobiUpdater.js build/$(PLUGIN_NAME)/updater/GoobiUpdater.js # build updater
			cat src/tmp/GoobiUtil.js >> build/$(PLUGIN_NAME)/updater/GoobiUpdater.js
			cp package.json build/$(PLUGIN_NAME)/package.json
			cp -r node_modules build/$(PLUGIN_NAME)/
			rm -rf src/tmp # clean tmp

			cp l10n/customDataTypeGoobi.csv build/$(PLUGIN_NAME)/l10n/customDataTypeGoobi.csv # copy l10n			
			
			cp src/webfrontend/css/main.css build/$(PLUGIN_NAME)/webfrontend/customDataTypeGoobi.css # copy css
